...
and multi-label binary classification by setting $\Ycal = \{-1,+1\}^k$.
...
Besides, it quite often leads to \textit{overfitting} particularly when the dimensionality of the feature space is high and the number of training examples is relatively small.
...
The {regularization theory} \citep{Evgeniou99a,Evgeniou02regularization} provides a framework to tackle those two problems.
...
For linear function class, there are several ways to define the regularization term among which $L_1$-norm and $L_2$-norm regularization are most popular.
...
\end{align*}
provides sparse parameter estimations such that we obtain a high dimensional weight vector with many zero entries.
This is an attractive property as feature selection is incorporated into the learning process and the resulting model is usually easy to interpret.
The $L_1$-norm regularization has been applied in, for example, \lasso\ \citep{Tibshirani94regression}.
Many other regularization techniques have been widely studied, to name but a few, $L_{1,2}$-norm regularization \citep{Argyriou07multitask}, and elastic net regularization \citep{Zou05regularizationa}.
