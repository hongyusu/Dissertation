...
In this chapter, we extend the structure output learning to the scenario where the output graph is hidden, by exploring the potential of learning from a random set of output graphs.
We start with a majority voting ensemble (\mve) of a set of structured output classifiers in Section~\ref{sc_su11}, and proceed with two aggregation techniques (\amm\ and \mam\ respectively) that performs inference before or after forming the ensemble.
In Section~\ref{sc_su14c}, we present and discuss the \rta\ method that improves \mam\ by the joint learning and inference on a random set of spanning trees.
