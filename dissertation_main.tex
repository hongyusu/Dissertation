...
Multilabel classification is a natural extension to single-label classification by defining multiple outputs associated with each input.
...
Flat multilabel classification is one branch of multilabel classification and has drawn increasing amount of interest from machine learning community \citep{Tsoumakas07multi,Tsoumakas10mining}.
...
There is another line of research on multilabel classification known as structured multilabel classification or structured output prediction where we define complex structure over multiple output variables.
Hierarchical classification is one type of structured output prediction in which the prediction needs to be addressed based on the hierarchical structure defined on multiple labels \citep{Silla11a}.
The hierarchical structure can be for example a rooted tree or a directed acyclic graph (\dag) with parent-children relationship.
...
We have also developed \spin\ that is able to predict a directed acyclic graph that connects multiple output variables, and applied in network response prediction problem.
