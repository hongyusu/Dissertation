...
\begin{definition}{Kernel Function.}
	A \textit{kernel} $k:\Xcal\times\Xcal\rightarrow\RR$ is a function that for all $x_i,x_j\in\Xcal$ satisfies
	\begin{align*}
		k(x_i,x_j) = \ip{\varphib(x_i)}{\varphib(x_j)},
	\end{align*}
	where $\varphib:\Xcal\rightarrow\vFcal$ is a feature map that encodes from an input space $\Xcal$ to a feature space $\vFcal$.
\end{definition}
% psd kernel
\begin{definition}{Positive Semi-definite Kernel.}\label{psdkernel}
	Given a non-empty set $\Scal=\{x_1,\cdots,x_m\}\subseteq\Xcal$ of $m$ objects, a function $k:\Scal\times\Scal\rightarrow\RR$ is called a positive semi-definite kernel if it is symmetric and positive semi-definite. That is
	\begin{align*}
		k(x_i,x_j) = k(x_j,x_i)\quad \text{and}\quad \sum_{x_i,x_j\in S}c_ic_jk(x_i,x_j)\ge0
	\end{align*}
	holds for any $c_i\in\RR$.
\end{definition}
\noindent
...
\begin{definition}{Kernel Matrix.}
	Given a kernel function $k$ and any choice of $m$ objects $\Scal=\{x_1,\cdots,x_m\}\subseteq\Xcal$, the $m\times m$ matrix $K=(k(x_i,x_j))_{i,j}$ is called a Kernel matrix (\textit{Gram matrix}) of kernel $k$ with respect to $\Scal$.
\end{definition}
\noindent
...
\begin{definition}{Positive Semi-definite Matrix.}
	...
	\begin{align*}
		...
		\end{align*} 
		for any $c_1,\cdots,c_m\in\RR$. If equality occurs only when $c_1=\cdots=c_m=0$, then $K$ is called \textit{strictly positive definite}. 
\end{definition}
\noindent
It is not difficult to show that Kernel matrices are positive semi-definite \citep{Taylor04kernel}.

\noindent


Definition~\ref{psdkernel} makes difference between general similarity measures and kernels.
The positive semi-definiteness is essential in kernel based methods from two perspectives.
First, the property ensures kernel based methods will converge to a relevant solution in convex optimization \citep{Boyd2004convex}.
Secondly, it is a key assumption in \textit{Reproducing Kernel Hilbert Space} theory described as follows.
...
In particular, for each unseen example $\vx$, \iblr\ first identifies the set of $K$-nearest neighbors $N_k(\vx)$ from the training data.
...
The \text{maximum spanning tree} approach takes the minimum number of edges that make a connected graph whilst maximizes the sum of edge weights.
...
On the other hand, the Maximum-of-Average-Marginals (\mam) aggregation first collects the local edge potentials $\psib_E^{(t)}$ from each base learner, averages them and performs a finial inference with the averaged edge potentials on a global consensus graph $\hat{G}=(\hat{E},V)$ where $\hat{E}=\bigcup_{t=1}^{T}{E^{(t)}}$ is the union of distinct edges of the set of random output graphs.
...
Meanwhile, the current structured output learning approaches rely on an output graph connecting multiple output variables to exploit the correlation between labels.
...
To serve as a starting point, the inference algorithm for \spin\ can be further developed, which not only needs to be scalable to the large scale social network datasets but should also have an approximation guarantee.
...
The additional weight can be interpreted as the affinity of an output graph to the training data, which can be used to select relevant output graph structures.
We need to study the corresponding optimization algorithm as the current alternative optimization \citep{Rakotomamonjy08simplemkl} is not efficient.
