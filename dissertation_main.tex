...
There is another line of research on multilabel classification known as \textit{structured multilabel classification} or \textit{structured output prediction} where we define complex structure over multiple output variables to model the dependency in a more comprehensive way.
...
The hierarchical structure can be for example a rooted tree in document classification problem \citep{} or a directed acyclic graph (\daggraph) with parent-children relationship in gene function prediction problem \citep{}.
There exists a large body of work on hierarchical classification




we assume complex interdependency on output variables.
The interdependency is defined by the structure of an output graph (e.g. Markov network) connecting multiple variables.
