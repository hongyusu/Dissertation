...
To measure the goodness of the estimator, we define a loss function (risk function) $\Lcal(\vy,f(\vx)):\RR\times\RR\rightarrow\RR^+$ between the true value $\vy$ and the predicted value $f(\vx)$.
Popular loss functions used in machine learning include \textit{Hamming Loss} (e.g., in \svmstruct\ of Section~\ref{sc_svmstruct})
\begin{align*}
	\Lcal_{0/1}(\vy,f(\vx)) = \ind{\vy=f(\vx)},
\end{align*}
\textit{Hinge Loss} (e.g., in \svm\ of Section~\ref{sc_svm})
\begin{align*}
	\Lcal_{hinge}(\vy,f(\vx)) = \maximize (0,1-\vy f(\vx)),
\end{align*}
\textit{Square Error Loss} (e.g., in Ridge Regression developed by \citet{Hoerl00ridge})
\begin{align*}
	\Lcal_{square}(\vy,f(\vx)) = (\vy, f(\vx))^2,
\end{align*}
\textit{Exponential Loss} (e.g., in \adaboost\ of Section~\ref{sc_boosting})
\begin{align*}
	\Lcal_{exp}(\vy,f(\vx)) = \exp(\vy f(\vx)),
\end{align*}
\textit{Logistic Loss} (e.g., in Logistic Regression of Section~\ref{sc_lr})
\begin{align*}
