...
We assume two random variables $\vx\in\vXcal$ and $y\in\Ycal$ are jointly distributed according to some fixed but unknown probability distribution $P(\vx,y)$ over domain $\vXcal\times\Ycal$.
...
The basic goal for \textit{statistical learning} is to provide an \textit{estimator} $f\in\Hcal:\vXcal\rightarrow\Ycal$ from a hypothesis class $\Hcal$ which predicts the value of $y$ given an input $\vx$.
...
\textit{Hinge Loss} e.g., in Support Vector Machines \citep{Cortes95support}
\begin{align}
	\Lcal_{hinge}(y,f(\vx)) = \maximize (0,1-y f(\vx)),\, \Ycal=[-1,+1], \label{hinge_loss}
\end{align}
\textit{$0/1$ Loss} e.g., in structured \svm\ \citep{THJA04}
\begin{align}
	\Lcal_{0/1}(y,f(\vx)) = \ind{y=f(\vx)},\, \Ycal = \ZZ, \label{one_loss}
\end{align}
\textit{Squared Loss} e.g., in Ridge Regression \citep{Hoerl00ridge}
\begin{align*}
	\Lcal_{squared}(y,f(\vx)) = (y-f(\vx))^2,\, \Ycal=\RR,
\end{align*}
\textit{Exponential Loss} e.g., in \adaboost\ \citep{Schapire99improved}
\begin{align}
	\Lcal_{exp}(y,f(\vx)) = \exp(-y f(\vx)),\, \Ycal=\RR, \label{exp_loss}
\end{align}
\textit{Logistic Loss} e.g., in Logistic Regression \citep{Chen99}
\begin{align}
...
The empirical risk minimization strategy is ill-posed.
Given training data, the strategy would provide infinity number of estimators with the same empirical risk.
...
In other words, underlying true distribution $P(\vx,y)$ is difficult to estimate from a finite sample of training examples.
...
\citepub{su14a} presents a structured output learning model for the network influence prediction problem.
The proposed model, \spin, is \textit{context-sensitive} which is different from previous the state-of-the-art methods which only model the influence in terms of the network connectivity.
The inference problem of \spin\ is \nphard\ in general.
We provides a semidefinite programming algorithm \sdp\ with approximation guarantee as well as a fast \greedy\ heuristics.
