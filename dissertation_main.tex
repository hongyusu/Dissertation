...
In flat multilabel classification, the multiple labels are essentially taken as a ``flat'' vector of labels.
...
\citet{Zhang05a,Zhang07mlknn} proposed Multilabel $K$-Nearest Neightbors (\mlknn) for flat multilabel classification.
\mlknn\ is an instance based learning approach \citep{Aha91instance} that is derived from $K$-Nearest Neightbor (\knn) algorithm designed for single-label classification problem.
...
Essentially \mlknn\ is a binary relevant classifier.
In other words, the algorithm ignores the label dependencies.

%
%
\subsection{Instant Based Logistic Regression (\iblr)}

\citet{Cheng09combining} developed Instance Based Logistic Regression (\iblr) with extension to flat multilabel classification.
...
The central idea of \iblr\ is to take the labels of the neighboring examples as features in order to evaluate the label of current example.
...
Classification Chains (\cc) developed in \citep{Read11classifier} is another problem transformation approach for flat multilabel classification.
...
\end{align*} 
\cc\ takes the label correlation into account by sharing label information between classifiers.

Probabilistic Classifier Chains (\pcc) presented in \citep{Read09classifier} explains the behavior of \cc\ in terms of condition probability theory.
Since there is no standard way for the classification order of microlabels in \cc, Ensemble Classifier Chains (\ecc) developed \citep{Read11classifier} further improves \cc\ by generating multiple chains of classifiers. 
...
\corrlog\ is a major step forward of \iblr\ by constructing a logistic regression classifier over all microlabels and by modeling pairwise microlabel correlation with a function defined on microlabels.

We will present \corrlog\ by first introducing the Independent Logistic Regression (\ilrs) model. 
...
\subsection{Ensemble Methods for Flat Multilabel Classification}
