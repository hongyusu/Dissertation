...
\textit{Hinge Loss} in e.g., Support Vector Machines \citep{Boser92,Cortes95support}
\begin{align*}
	\Lcal_{hinge}(y,f(\vx)) = \maximize (0,1-y f(\vx)),\, \Ycal=[-1,+1],
\end{align*}
\textit{Hamming Loss} in e.g. structured \svm\ \citep{THJA04,TJTA05}
\begin{align*}
	\Lcal_{0/1}(y,f(\vx)) = \ind{y=f(\vx)},\, \Ycal = \ZZ,
\end{align*}
\textit{Squared Loss} in e.g., Ridge Regression \citep{Hoerl00ridge}
\begin{align*}
	\Lcal_{squared}(y,f(\vx)) = (y-f(\vx))^2,\, \Ycal=\RR,
\end{align*}
\textit{Exponential Loss} in e.g., in \adaboost\ \citep{Schapire99improved}
\begin{align*}
	\Lcal_{exp}(y,f(\vx)) = \exp(-y f(\vx)),\, \Ycal=\RR,
\end{align*}
\textit{Logistic Loss} in e.g., in Logistic Regression \citep{Chen99}
\begin{align*}
	\Lcal_{log}(y,f(\vx)) = \log(1+\exp(-y f(\vx))),\, \Ycal=[-1,+1].
\end{align*}
We will study the loss functions with the corresponding learning models into detail in later part of the thesis.
...
However, the distribution $P(\vx,y)$ that generates the examples is unknown.
