...
The bias term as in (\ref{}) is omitted which is equvilent to augment $\vx$ with a constant term \citep{Bian12corrlog}.
Otherwise, (\ref{ilrs}) can be derived into (\ref{}) by replacing $\vw$ with $\frac{\vw}{2}$.
...
\begin{align*}
	Q(\vy) = \exp\left\{\sum_{k<j}\alpha_{k,j}\vy[k]\vy[j]\right\}.
\end{align*}
The formalism of \corrlog\ is
\begin{align*}
...
\end{align*}
Thus, \corrlog\ examines the label correlation by augmenting joint prediction with a quadratic term $Q(\vy)$.


%
%
\subsection{Classifier Chain (\cc)}
