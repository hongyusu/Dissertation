...
To measure the goodness of the estimator, we define a loss function $\Lcal(y,f(\vx)):\Ycal\times\Ycal\rightarrow\RR^+$ as a monatomic bounded function between the true value $y$ and the predicted value $f(\vx)$.
...
\end{align}
As a result, we should search for an estimator $f$ that minimizes the true risk defined by (\ref{true_risk}).
...
There also exists the line of research that aims to optimize Logistic Regression from its dual representation.
...
Given that $\vw$ achieves correct separation on the training data, we can decide the label of a test example $\vx_{ts}$ by decision rule $y_{ts}=\sign(f(\vx_{ts}))$.
...
With margin slack parameter, the new goal is to maximize the margin while penalize the data points which either lie on the wrong side of the hyperplane or have margin less than one.
...
This section will be devoted to bagging and boosting since both methods are extensively studied and are most related to this thesis.
...
\citet{Schapire90the} has proved that two classes of learnability are equal which lays the foundation of the boosting algorithm that aims to tune a weak learning algorithm into a strong one that achieves high performance.
...
In addition, \citet{Schapire99improved} proposed a variant of the algorithm which updates the adaptive parameters in order to minimize the exponential loss (\ref{exp_loss}) of the weak learners.
...
This chapter will be devoted to flat multilabel classification in which several classical algorithm techniques will be presented.
...
For problem transformation category, we will present the methods which aim to transfer the flat multilabel classification problem into other well studied problem settings (e.g., single-label classification problem, ranking problem).
For algorithm adaptation category, we will describe the methods which directly modify the popular learning techniques (e.g., Logistic Regression and ensemble method) to solve the multilabel classification problem.
...
\mlknn\ transforms the flat multilabel classification problem into a set of single-label classification and processes one microlabel independently.
...
In particular, we define the output graph $G=(E,V)$ by a node set $V=\{1,\cdots,k\}$ corresponding to the microlabels $\{y_1,\cdots,y_k\}$ and an edge set $E=V\times V$ representing the dependency between pair of microlabels.
...
After weight parameter $\vw$ is obtained, one needs to solve the \textit{argmax} problem to find the best output for a given input $\vx$
\begin{align}
...
\mmmn\ extends \svm\ (Section~\ref{sc_svm}) to structured output space.
...
%
%
\subsection{Max-Margin Conditional Random Fields (\mmcrf)}

Max-Margin conditional random field (\mmcrf) is a structured output prediction methods developed in \citep{Rousu07}.
\mmcrf\ is a sibling method of \mmmn\ in which it extends \mmmn\ with a general and efficient optimization strategy. 
...
\end{align*}
where $\phib_e(\vx,\vy_e)=\varphib(\vx)\otimes\Gamma_e(\vy_e)$ is the tensor product of the input feature and the output feature of a pair of microlabels with respect to edge $e\in E$ in the output Markov network $G$.
...
The inference problem (Definition~\ref{mmcrfinference}) is solved by loopy belief propagation (\lbp) on general graph which is an instantiation of the message-passing algorithm developed by \cite{Wainwright08graphical}.
...
The proposed model, \spin, is \textit{context-sensitive} which is different from the previous the state-of-the-art methods which only model the influence in terms of the network connectivity.
...
%
\subsection{Methods}
We approach the problem by structured output learning, where we define a computability score as an inner product of the action $\va$ and the response network $\Gva$
\begin{align*}
...
The \iqp\ is then tackled by similar technique introduced in \citep{Geomans1995improved}, such that each variable $x_u$ is relaxed to a vector $\vv_u\in\RR^n$ and the relaxed problem is solved by semidefinite programming (\sdp). 
...
As a result, the \greedy\ algorithm starts with an empty vertex set and adds one vertex at a time such that the increment of the score is maximized over all inactivated vertices.
...
Structured output learning builds an output graph connecting multilabel interdependent output variables to model label correlation in a comprehensive manner. 
...
Molecular classification, the goal of which is to predict the anti-cancer potentials of the drug-like molecules, is one crucial step in drug discovery and has drawn enormous attention from machine learning community \citep{Singh12qsar,Dutt12classification}.
...
To efficiently and accurately predict molecular activities in multiple cell lines at the same time, we applied a structured output learning approach in \citepub{su10} which to our knowledge is the first multilabel learning approach for molecular classification problem.
...
The parameter $\vw$ is learned by maximizing the minimum loss-scaled margin between correct examples $(\vx_i,\vy_i)$ and incorrect pseudo-examples $(\vx_i,\vy)$ over the whole training set, which amounts to solving the optimization problem in (Definition~\ref{def_mmmn})
...
In addition, we denote by $G_{\times}(G_1,G_2)$ the direct product graph of two graphs $G_1$ and $G_2$, of which the vertices of the product graph are computed from 
\begin{align*}
...
Using exponential series or geometric series walk kernel can be computed in cubic time \citep{Gartner03a} by
\begin{align*}
...
The kernel weights identical part in two graphs with contextual information.
...
In \citepub{su11}, we explore the potential of constructing a majority voting ensemble of multiple structured output learners and examine the performance of the proposed model on molecular activity classification problem.
...
It is also worth pointing out that the \mve\ framework is not restricted to base classifier \mmcrf\ and can be extended with any structured output learning models as long as they incorporate the output structure into learning process and make predictions based on the output structure.
...
Besides, \citepub[Theorem 5]{su14c} suggests we should, instead of optimizing the margin separately on each spanning tree as in \mam, optimize the joint margin from all spanning trees.
...
However, it is not clear on how to preserve label distribution under multilabel classification setting.
...
The stratification scheme ensures that smaller classes also have representations in all folds. 
...
The goal of the test procedure is to examine whether a pair of algorithms are significantly different from each other under multiple hypothesis test settings.
...
\begin{enumerate}
	...
	\item \spin\ developed in \citepub{su14a} is a structured output learning algorithm for multilabel classification with observed output graph. \spin\ can predict a direct acyclic graph (\daggraph). \spin\ is applied to network response prediction problem within the context of social network analysis.
	\begin{enumerate}
	...
	\item The structured output learning algorithm used in \citepub{su10} for multilabel molecular activities classification problem is an instantiation of \mmcrf\ \citep{Rousu07}.
	\begin{enumerate}
		\item The code is implemented in \matlab.
...
The approach has difficulty to model label dependency.
...
The study not only widens the applicability of structured output learning, but also demonstrates accurate prediction can be achieved by structured output learning when the output dependency structured can be obtained and utilized during learning.

Meanwhile, we realize that current structured output learning approach relies on an output graph connecting multiple output variables in order to model the label dependency in a comprehensive manner.
...
As the first direction of applying the presented algorithms is clear and application oriented, we will concentrate on the algorithm development and theoretic analysis part.
...
\rta\ combines the feature weight parameter from each spanning tree via a $L_2$-norm convex combination.
This can be further extended by applying $L_1$-norm combination.
Learning with $L_1$-norm regularization of parameter combination has been previously studied in the setting of multiple kernel learning \citep{Rakotomamonjy08simplemkl}, which can be formulated into an equivalent form of learning a weighted $L_2$-norm combination.
The weight of the parameter vector on each spanning tree can be explained as the fitness of the tree to the current training data.
This can be used to select output graph that is most relevant to the current problem.
We should also design the optimization techniques for $L_1$-norm regularization as current alternative optimization approach in \citep{Rakotomamonjy08simplemkl} is not efficient.
