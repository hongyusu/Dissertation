...
\textit{Hierarchical classification} is one type of structured output prediction in which the prediction needs to be addressed along the pre-established hierarchical structure defined on multiple labels \citep{Silla11a}.
The hierarchical structure can be a rooted tree for example in document classification problem \citep{}, or a directed acyclic graph (\daggraph) with parent-children relationship for example in gene function prediction problem \citep{}.
There exists a large body of work on hierarchical classification from the early work that use the hierarchical information in a heuristic manner \citep{} to the recent work that encodes the  hierarchy into learning process \citep{}.

\textit{Graph labeling classification} is another type of structured output prediction in which the structure of the output graph can take any general form.
As the output graph structure of graph labeling classification is not restricted to hierarchy, there is no concept of ``level'' as parent-child relation if hierarchical classification. 




The structured output prediction models usually incorporate the output dependency structure into the learning process in order to improve the classification performance \citep{lafferty01,taskar02,Taskar04max,THJA04}.
