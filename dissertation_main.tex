...
During each iteration, the algorithm compute a weak learner $f^t(\vx)$ base on all training data and current distribution $D^{t}$ (line~\ref{compuet_f}), compute the weighted training error $\epsilon^k$ for the current weak learner (line~\ref{compute_error}), and compute the adaptive parameter $\alpha^t$ for current weak learner (line~\ref{compute_alpha}).
...
\begin{algorithmic}[1]
	...
	\FOR{$t=1\cdots T$}
		...
		\STATE $\alpha^{t} = \frac{1}{2}\log\left(\frac{1-\epsilon^k}{\epsilon^k}\right)$ \label{compute_alpha}
