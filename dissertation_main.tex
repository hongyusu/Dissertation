...
Besides the proposed structured output learning models, the thesis contains the theoretical studies which not only explain the improvement in terms of the prediction performance obtained by the new models but also provide the generalization error analysis.
...
This can be used to select the output graphs that are relevant to the current training data.
We should also design the optimization techniques for $L_1$-norm regularization as the current alternative optimization approach proposed in \citep{Rakotomamonjy08simplemkl} is not efficient.
