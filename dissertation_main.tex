...
It is also important to obtain an approximation guarantee for the new inference algorithm in order to ensure the quality of the inference.
...
Next, we plan to investigate the possibility of learning the feature weight parameters defined on a collection of random spanning trees through a $L_1$-norm regularized combination.
Learning with the $L_1$-norm regularized parameter combination has previously been studied in multiple kernel learning \citep{Rakotomamonjy08simplemkl}, which can be formulated equivalently as learning a weighted $L_2$-norm combination.
The weight can be interpreted as the affinity of an output graph to the training data, which can be used to select relevant output graph structures.
