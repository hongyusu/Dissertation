...
The hierarchy can be a rooted tree for example in document classification problem \citep{Hao07hierarchicaly,Li07hierarchical,Rousu06}, or a directed acyclic graph (\daggraph) with parent-children relationship for example in gene function prediction problem \citep{Barutcuoglu06hierarchical}.
...
\textit{Graph labeling} is another type of structured output prediction in which the output graph can take arbitrary form (e.g. Markov network).
It is not necessary to have hierarchical structure and does not have a concept of ``level'' as in hierarchical classification.
The structured output prediction models often directly incorporate the output graph into learning and use the dependency or the correlation to improve the classification performance \citep{lafferty01,taskar02,Taskar04max,THJA04}.
