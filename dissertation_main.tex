...
The flat multilabel classification render the difficulty of modeling the complex dependency structure.
...
We have designed new learning framework that does not required the observed output dependency structure, instead can learn from a random sample of spanning trees as the output graph.
...
In this thesis, we studied the supervised learning for classification.
In particular, we focused on multilabel classification where the task is to prediction multiple interdependent output variables given an input example.
...
Structured output learning approach is a natural remedy to flat multilabel classification.
The first results of the thesis is to extend the structured output learning methods.
In particular, we have apply the structured output learning on molecular activity classification problem.
We have also developed \spin\ that is able to predict a directed acyclic graph that connects multiple output variables.
\spin\ has beed used in network influence prediction problem.
The methods and the empirical results have shown that structured output learning is an important extension to flat multilabel classification approach for predicting multiple interdependent output variables.


In this thesis, we focus on structured output learning approach for multilabel classification.
There are two major outcomes from the thesis.
The first outcome is we have put forward new structured output learning methods for multilabel classification, relying on representing the label correlation as an output graph.
We have both applied the structured output learning on molecular classification problem and developed new method \spin\ for network influence prediction problem.
In particular, \spin\ is able to predict a direct acyclic graph that connects multiple output variables and can be used to study 


The proposed models have favourable prediction performance on two real world multilabel classification problem, namely the molecular activity classification and the snetwork influence prediction.
The methods and the empirical studies shows that the structured output learning improves the flat multilabel classification approach by an output graph that models the correlation of the output variables.

The second outcome of the thesis is that we have developed the methods for structured output learning when the output dependency structure is not observed.
The methods can learn from a random set of spanning trees 
...
In particular, we have developed new learning algorithms that incorporates the output dependency structure into learning process.
The new models gain advantage of modeling label correlation.
We have examined the performance of the proposed methods in two real world multilabel classification tasks, namely molecular activity prediction and network influence prediction.
In addition, we have studied the problem of the structured output learning where the output graph is not observed.
We have designed the new learning framework that is able to learn from a random sample of spanning trees, which widen the applicability of the structured output learning.
We have also explored the efficient inference algorithm.
As a result, the proposed model can often avoid the exponential sized parameter and difficult inference problem.
...
As the first direction of applying the presented algorithm is clear and more application oriented, we will concentrate on the algorithm development and theory analysis part.

As a starting point, we can extend \spin\ model by developing new inference algorithm that is both scalable and has apprimation guarantee.
Besides, the current research ends with \rta\ method that performs joint learning and inference on a random sample of spanning trees.
