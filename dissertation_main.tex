...
In this section, the author will introduce two fundamental concepts in statistical machine learning, known as \textit{empirical risk minimization} \citep{Vapnik92principles} and \textit{regularization} \citep{Evgeniou99a} which create most learning algorithms presented in the remaining part of this thesis.
...
The model thus created is usually easy to interpret.
...
The framework of \svm\ was originally introduced by \citet{Cortes95support}.
...
Based on the maximum-margin principle, the goal of learning is to find a separating hyperplane such that it maximizes the margin with respect to all training examples while separating the training examples into two classes.
...
Any kernel function can be represented as an inner product in some feature space $\vFcal$.
...
\begin{definition}{Positive Semi-definite Matrix.}
	...
	\begin{align*}
		...
		\end{align*} 
		for any $c_1,\cdots,c_m\in\RR$.
		% If equality occurs only when $c_1=\cdots=c_m=0$, then $K$ is called \textit{strictly positive definite}. 
\end{definition}
\noindent
...
%\textit{Mercer's Theorem} \citep{Taylor04kernel} has shown that every positive semi-definite symmetric function is a kernel.
...
Adaptive Boosting (\adaboost) proposed by \citet{Freund97a} is the very first practical boosting algorithm and is the most influential one.
...
\textit{Bootstrap Aggregating} (\bagging) \citep{Breiman96bagging} is an ensemble method that exploits the independency between weak learners.
...
% revision on structured output prediction definition
...
A vector $\vy=(\vy[1],\cdots,\vy[k])\in\vYcal$ is called a \textit{multilabel} and its element $\vy[j]$ is called a \textit{microlabel}.
...
The goal of learning is to find a mapping function $f\in\Hcal:\vXcal\rightarrow\vYcal$ which can compute the best multilabel for an input example such that the predefined loss function $\Lcal$ for the unseen examples will be minimized.
...
Multilabel $K$-Nearest Neightbors algorithm (\mlknn) developed by \citet{Zhang05a,Zhang07mlknn} is perhaps the most famous binary relevance classifier for flat multilabel classification.
\mlknn\ is also an instance based learning approach \citep{Aha91instance} that is derived from the $K$-Nearest Neightbors algorithm (\knn) designed for single-label classification.
...
For each unseen example $\vx$, \mlknn\ first identifies a set of $K$-nearest neighbors $N_K(\vx)$ from the training set.
...
Classifier chains (\cc) \citep{Read09classifier,Read11classifier} is another problem transformation approach for flat multilabel classification.
...
\adaboostmh\ \citep{Schapire99improved,Esuli2008boosting} is a multilabel variant of the \adaboost\ algorithm.
The core idea of \adaboostmh\ is to apply the hamming loss instead of the $0/1$ loss (\ref{one_loss}). 
...
\begin{algorithmic}[1]
	\REQUIRE Training sample $\Scal = \{(x_i,\vy_i)\}_{i=1}^m$, learning function $\Wcal$, number of weak learners $T$
	\ENSURE Boosting ensemble $H(\vx)$
	\STATE Initialize $D^1(i,l)=\frac{1}{mk},\, \forall i\in\{1,\cdots,m\},l\in\{1,\cdots,k\}$
	\FOR{$t=1\cdots T$}
...
\begin{align}
	...
	= \prod_{j=1}^{k}\frac{\exp(\vy[j]\vw^{\tp}\varphib(\vx))}{\exp(\vw^{\tp}\varphib(\vx))+\exp(-\vw^{\tp}\varphib(\vx))}, \label{ilrs}
\end{align}
where $j$ is the index that iterates over microlabels and $\tp$ denotes matrix transpose.
...
\begin{align*}
	P_{\text{\corrlog}}(\vy|\vx) &\propto P_{\text{\ilrs}}(\vy|\vx) Q(\vy) \\
		&=\exp\left\{ \sum_{j=1}^{m}\vy[j]\ip{\vw}{\varphib(\vx)} + \sum_{k<j}\alpha_{kj}\vy[k]\vy[j] \right\}.
\end{align*}
...
\begin{align*}
	h(\vx) = \ip{U}{\varphib(\vx)},
\end{align*}
where $\varphib(\vx)\in\RR^d$ is the input feature map in the original feature space and $U\in\RR^{d\times d}$ is a square matrix.
...
\begin{definition}{\mtl\ Optimization Problem in Primal}\label{mtl_opt}
	\begin{align*}
		\underset{\substack{U\in\RR^{d\times d}\\A\in\RR^{d\times T}}}{\minimize} \left\{\sum_{t=1}^{T}\sum_{i=1}^{m}\ell(\vy_{i,t},\ip{\va_t}{\ip{U}{\varphib(\vx_i)}})+C\norm{A}_{2,1}^2\right\},
	\end{align*}
\end{definition}
\noindent
...
\citet{Argyriou08convex} developed an extension of \mtl\ that introduces a nonlinear generalization using kernel methods.
...
In particular, we define the output graph $G=(E,V)$ by a node set $V=\{1,\cdots,k\}$ which corresponds to the microlabels $\{\vy[1],\cdots,\vy[k]\}$ and an edge set $E=V\times V$ which represents the correlation between microlabels.
For an edge $e=(j,j')\in E$, we use $\vy_e$ to denote the label of the edge $e$ with respect to a multilabel $\vy$ by concatenating the head label $\vy[j]$ and the tail label $\vy[j']$, with an edge label domain $\vy_e\in\vYcal_e=\Ycal_j\times\Ycal_{j'}$.
...
The model assumes a score function $\ip{\vw}{\phib(\vx,\vy)}$ as the inner product between a feature weight parameter $\vw$ and a joint feature map $\phib(\vx,\vy)$,
In particular, $\phib:\vXcal\times\vYcal\rightarrow\RR^d$ maps an input--output pair to a vector of $d$ dimension.
...
\end{align}
The update (\ref{sp_update}) usually leads to over-fitting.
...
Condition random field (\crf) \citep{lafferty01,taskar02} is a discriminative framework that constructs a conditional probability $P(\vy|\vx)$ for an input variable $\vx\in\vXcal$ and an output variable $\vy\in\vYcal$.
It optimizes the log-loss which is analogue to the $0/1$ loss (\ref{one_loss}) in the structured output space.
...
Let $G=(E,V)$ denote an output graph such that $\vy = (\vy[v])_{v\in V}$.
...
\end{align}
When conditioned on $\vx$, random variables $\vy[v]$ obey the Markov property with respect to the output graph $G$.

Applying the similar regularization technique as used in logistic regression in Section~\ref{sc_lr}, the feature weight parameter $\vw$ can be solved by introducing a Gaussian prior and maximizing the logarithm of the resulting \textit{maximize a-posteriori} (\map) problem \citep{taskar02}
\begin{align}
...
Nevertheless, \mmmn\ improves \crf\ by avoiding the evaluation of the partition function (\ref{crf_partition_function}) and allowing complex loss functions to be defined.
...
To solve the optimization problem, \mmcrf\ uses the conditional gradient optimization method \citep{Bertsekas95nonlinear} in the marginalized dual space \citep{Taskar04max}, which not only benefits from a polynomial-size parameter space but also enables kernels (Definition~\ref{kernels}) that can deal with the non-linearity of the complex input space.
...
We develop a semi-definite programming algorithm (\sdp) with an approximation guarantee as well as a fast \greedy\ heuristics.
...
See \citepub{su14a} for an example of input and output feature maps.
...
\end{align}
As these two problems are different only in terms of the definition of scores, we explain our inference algorithm based on (\ref{spin_inference}) by writing the problem explicitly in terms of the weight vectors and the feature maps
\begin{align}
...
\end{align}
where $s_{\vy_e}(e,\va) = \sum_{i}\vw_{i,e,\vy_e}\varphib(\va)[i]$ denotes the score of an edge $e$ with an edge label $\vy_e$.
The tuples $(i,e,\vy_e)$ index the elements in the vector $\vw$.
...
The first is called {\sdp\ inference} which introduces for each node $u\in V$ a binary variable $x_u\in\{-1,+1\}$ and transforms the inference problem into an integer quadratic programming problem (\iqp).
The \iqp\ is tackled by a similar technique proposed by \citet{Geomans1995improved} such that each variable $x_u$ is relaxed to a vector $\vv_u\in\RR^n$ and the relaxed problem is solved by semi-definite programming (\sdp). 
...
As \sdp\ inference is not scalable to large scale networks, we develop a \greedy\ heuristic based on the observation stated in the following lemma:
\begin{lemma}
...
\end{lemma}
\noindent
The proof and the definition of the marginal gain function are given in the supplementary material of \citepub{su14a}. 
...
Structured output learning relies on an output graph connecting multilabel interdependent output variables to exploit the correlation between labels.
...
The first challenge is the scalability issue in which a set of single-label classifiers needs to be built to predict multiple activities of a molecule.
...
To apply structured output learning, we extract output graphs from several auxiliary datasets which encode the correlation between multiple molecular activities.
...
\begin{align*}
	...
	&\quad \xi_i>0, \forall i\in\{1,\cdots,m\}, \forall \vy\in\vYcal,
\end{align*}
...
The \text{maximum spanning tree} approach takes the minimum number of edges that make a connected graph whilst maximizing the sum of edge weights.
The \text{correlation thresholding} approach takes all edges that exceed a fixed threshold in terms of the pairwise correlation, which typically generates a non-tree graph.
...
\begin{align*}
	F_j^{\text{\mve}}=\underset{\vy[j]\in\Ycal_j}{\argmax}\,\left(\frac{1}{T}\sum_{i=1}^{T}\ind{F_j^{(t)}(x)=\vy[j]}\right), \forall j\in\{1,\cdots,k\},
\end{align*}
...
The \text{random spanning tree} approach first generates a random correlation matrix and extracts a spanning tree out from the matrix, which outputs a tree structure connecting all vertices.
The \text{random pairing} approach randomly draws two vertices at a time and couples the two with an edge, which outputs a set of disconnected pairs.
...
\end{align*}
In words, the max-marginal is the maximum score of a multilabel consistent with $\vy[i]=u_i$.
...
\begin{align*}
	...
	= \underset{\vy \in \vYcal}{\argmax}\, \frac{1}{T} \sum_{t=1}^T \sum_{e\in \hat{E}} \ip{\vw_e^{(t)}}{\phib_e(x,\vy_e)}. %\label{ensemble3}
\end{align*}
...
\begin{align*}
F^{\text{\tiny MAM}}(x) 
 & =\underset{\vy \in \vYcal}{\argmax}\, \frac{1}{T} \sum_{t=1}^T \sum_{i,e,\vu_e} \mu^{(t)}(i,e,\vu_e) \cdot H_e(i,\vu_e;x,\vy_e)\nonumber \\
 & =\underset{\vy \in \vYcal}{\argmax}\,   \sum_{i,e,\vu_e}  \bar\mu(i,e,\vu_e) H_e(i,\vu_e;x,\vy_e),
\end{align*}
...
In particular, \citepub[Theorem 1]{su14b} states that the reconstructive error of \mam\ is guaranteed to be less than or equal to the average reconstruction error of base classifiers.
...
Starting from a sampling results, \citepub[Lemma 3]{su14c} shows that with high probability a big fraction of the margin achieved by a complete graph can be obtained by a conical combination of a random sample of spanning trees.
The number of the spanning trees does not need to be large.
...
Thus, in addition to \citepub[Theory 1]{su14b}, we further provide the theoretical justification of combining a set of base learners trained on a collection of random output graphs. 
...
Let $\Tcal=\{T_1,\cdots,T_n\}$ denote a sample of $n$ random spanning trees, and $\{\vw_{T_t}|T_t\in\Tcal\}$ denote the feature weight parameters to be learned on each tree.
...
\citepub[Lemma 8]{su14c} guarantees that with high probability the global maximizing multilabel is in the list and $K$ does not need to be large.
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item The package can be found from \url{https://github.com/hongyusu/RTA}.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item The package can be found from \url{https://github.com/hongyusu/SPIN}.
	\end{enumerate}
	...
	\begin{enumerate}
		...
		\item The packages can be found from \url{https://github.com/hongyusu/RandomOutputGraphEnsemble}.
	\end{enumerate}
\end{enumerate}
...
In particular, the proposed algorithm \spin\ can predict a directed acyclic graph from an observed underlying network which best ``responds'' to an input example.
...
Current structured output learning approaches rely on an output graph connecting multiple output variables to exploit the correlation between labels.
...
The work presented in the thesis will be extended along two main directions.
First, the algorithms developed in this thesis can be applied to other multilabel classification problems in which the output graph does not need to be observed but is believed to play an important role during learning.
Secondly, the development of the learning algorithms and the theoretical studies are readily to be continued.
As the first research direction is application oriented, we will focus on the latter part.

To serve as a starting point, the inference algorithm for \spin\ can be further developed such that the model can be applied to large scale social network datasets.
It is also important to obtain an approximation guarantee for the new inference algorithm in order to ensure the quality of the solution.
Secondly, we plan to study \rta\ in the multilabel classification problems where the output graph takes a more general form.
The setting is interesting as in many real world problems the underlying output graph structure is usually more complex than a spanning tree or a chain but should be much simpler than a complete graph.
The exact inference is also prohibitive for any polynomial time algorithms.
To tackle the problem we plan to randomly sample spanning trees from the general graph structures rather than a complete graph.
In particular, we are interested in the properties of the new algorithm (e.g., the generalization error bound, conditions for exact inference).
We also need to develop the algorithm that allows to generate spanning trees uniformly at random.
Next, we plan to investigate the possibility of learning a convex combination of a set of random spanning trees.
Compared to the current conical combination, this approach will lead to a different objective function that is similar to the $L_1$-norm regularized parameter combination previously studied in multiple kernel learning \citep{Rakotomamonjy08simplemkl}.
The objective function can be expressed equivalently as learning a weighted $L_2$-norm regularized parameter combination.
The weight can be interpreted as the affinity of an output graph to the current training data.
We can use the weights to select relevant output graph structures.
We need to study the corresponding optimization algorithm as the current alternative optimization developed by \citet{Rakotomamonjy08simplemkl} is not very efficient.
The theoretical analysis of \rta\ should also be extended to the new algorithm.
