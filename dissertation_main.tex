...
In particular, the algorithm compute in each iteration a weak learner $f^t(\vx)$ base on all training examples and the current distribution $D^{t}$ (line~\ref{compuet_f}), compute the weighted training error $\epsilon^k$ for the current weak learner (line~\ref{compute_error}), and compute the adaptive parameter $\alpha^t$ for current weak learner (line~\ref{compute_alpha}).
...
In particular, the algorithm reduces from a multilabel classification problem to a set of single label classification problem by replacing each training example $(\vx_i,\vy_i)$ with $k$ examples $\{(\vx_i,\vy_i[l])\}_{l=1}^k$.
...
% TODO: adaboost multilabel variant
