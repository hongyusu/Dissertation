...
For each microlabel $\vy[j]$, \cc\ first constructs a new training data $S_j$ by taking the $j$'th microlabel as the output variable and combing the feature space of $\vx$ with all $j-1$ prior microlabels as input features, which is defined by
\begin{align*}
	S_j = \{((\vx_i[1],\cdots,\vx_i[d],\vy_i[1],\cdots,\vy_i[j-1]),\vy_i[j])\}_{i=1}^{m}.
\end{align*}
A classifier $h_j$ is built by applying any single-label classifciation algorithm on $S_j$.
