...
This is called \texit{single-label classification} if the output variable is discrete, or \texit{regression} if the output variable is continuous.
...
There exists another line of research in multi-label classification known as \textit{structured output prediction} where a complex structure (output graph) is defined on multiple output variables to model dependencies in a more comprehensive way.
...
In addition, we develope a joint learning and inference framework that is based on Max-Margin learning on a random sample of spanning trees.
...
multi-label binary classification problem by setting $\Ycal = \{-1,+1\}^k$.
...
To measure the goodness of the estimator, we define a loss function $\Lcal(y,f(\vx)):\Ycal\times\Ycal\rightarrow\RR^+$ as a monotonic bounded function between the true value $y$ and the predicted value $f(\vx)$.
