...
The hierarchy can be a rooted tree for example in document classification problem \citep{Hao07hierarchicaly,Li07hierarchical,rousu07}, or a directed acyclic graph (\daggraph) with parent-children relationship for example in gene function prediction problem \citep{Barutcuoglu06hierarchical}.
There exists a large body of work on hierarchical classification from the early work that uses the hierarchical structure heuristically for preprocessing or post-processing \citep{Koller97hierarchically,Dumais00Hierarchical,Liu05support,DeCoro07bayesian} to the recent work that encodes the structure into learning \citep{Cai04hierarchical,cesabianchi05incremental,Gopal12bayesian}.
