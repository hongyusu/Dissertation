...
When the multiple output variables are treated as a ``flat'' vector, the problem is often known as flat multilabel classification.
\textit{Flat multilabel classification} is one branch of multilabel classification and has drawn increasing amount of interest from machine learning community \citep{Tsoumakas07multi,Tsoumakas10mining}.
As multiple output variables can be ``on'' and ``off'' simultaneously, various flat multilabel classification algorithms have been developed that aim to explore the correlation or the dependency between multiple output variables in order to make better prediction \citep{Schapire99improved,Zhang05a,Read09classifier,Cheng09combining,Bian12corrlog}.
...
There is another line of research on multilabel classification known as \textit{structured multilabel classification} or \textit{structured output prediction} where we define a complex structure (output graph) over multiple output variables to model the dependency in a more comprehensive way.
\textit{Hierarchical classification} is one type of structured output prediction in which the prediction needs to be reconciled along the pre-established hierarchical structure on multiple labels \citep{Silla11a}.
Hierarchical classification is usually applied on the problem with different level of granularity. 
The hierarchy can be either a rooted tree for example in document classification problem \citep{Hao07hierarchicaly,Li07hierarchical,Rousu06}, or a directed acyclic graph (\daggraph) with parent-children relationship for example in gene function prediction problem \citep{Barutcuoglu06hierarchical}.
There exists a large body of work on hierarchical classification from the early work that uses the hierarchical structure heuristically for preprocessing or post-processing \citep{Koller97hierarchically,Dumais00Hierarchical,Liu05support,DeCoro07bayesian} to the recent work that encodes the structure into learning \citep{Cai04hierarchical,Cesabianchi05incremental,Rousu06,Gopal12bayesian}.

\textit{Graph labeling} is another type of structured output prediction in which the output graph often takes more general form and does not have the concept of ``level'' compared to hierarchical classification.
The output graph structure can be for example a sequence \citep{collins02a} or a Markov network \citep{Taskar04max}.
...
Figure~\ref{} illustrates the taxonomy of the multilabel classification.
However it is worthy of mentioning that there is no clear line drawn between different taxonomies as some learning algorithms might belong to different categories, for example the algorithms proposed in \citep{THJA04,Rousu06}.
In addition, we focus on the graph labeling approach in this thesis.
Without loss of generality, when we use the term ``structured output prediction'' we explicitly means ``graph labeling'' in the following part of the thesis.

One inherited with structured output learning is the output dependency structure is assumed to be pre-established.
...
Besides, we study the theoretic aspects of the proposed models in order to gain insights into the behavior of the models.
...
The contributions of the thesis first lie in the development of the new structured output learning methods for multilabel classification problems.
...
In particular, it suggests to minimize 
 \begin{align*}
	\Jcal(f) = \Rcal_{emp}(f) + \lambda\Omega(f),
\end{align*}
...
The idea of Logistic Regression is also applied in many multilabel classification algorithms presented in the later part of the thesis (e.g. Section~\ref{sc_iblr}, Section~\ref{sc_corrlog}, Section~\ref{sc_mmmn}).
...
Besides decision boundary, Logistic Regression can output the class probability as the the ``distance'' of the data point to the decision boundary.
It is the probabilistic output that makes Logistic Regression no more than a classifier.

% optimization
The parameter $\vw$ and $b$ of Logistic Regression can be obtained by maximizing the probability (likelihood) of the training data.
...
Though \mle\ training of Logistic Regression can generate a model that fits the training data, it does not guarantee to generalize well on the test data.
...
% some history
Support Vector Machines (\svm) is probably the most widely used single-label classification algorithm in many real world applications.
Its extensions to multilabel classification problem will be described in the label part of the thesis (e.g. Section~\ref{sc_svmstruct}).
In this section we will start the story of \svm\ by introducing the \textit{Maximum-Margin Principle}, which is also the basis of many structured output learning models (e.g. Section~\ref{sc_mmmn}, Section~\ref{sc_svmstruct}, Section~\ref{sc_su14b}, Section~\ref{sc_su14c}).
...
The algorithm framework of \svm\ was first introduced in \citep{Boser92,Cortes95support}.
...
\begin{definition}{Dual Soft-Margin \svm\ Optimization Problem.}\label{softsvmdual}
	...
	\end{align*}
\end{definition}
It is not difficult to verify that according to KKT conditions only examples with both $\xi_i=0$ and satisfying equality constraints in (\ref{softsvmconstraint}) will be ``active'' and have dual variable $\alpha_i>0$.
...
The parameter $\vw$ is learned by maximizing the minimum loss-scaled margin between correct examples $(\vx_i,\vy_i)$ and incorrect pseudo-examples $(\vx_i,\vy)$ over the whole training set
\begin{align*}
	\underset{\vw,\vxi_i}{\minimize} & \quad \frac{1}{2}\norm{\vw}^2 + C\sum_{i=1}^{m}\xi_i\\
...
To build the output graph, we first compute the covariance matrix for cell lines based on auxiliary data, then extract the network structure by applying the following two methods.
...
Structured output learning (Chapter~\ref{ch_sop}) gains the power of the representation by connecting multiple output variables by an output graph.
In this chapter, we extend the structured output learning to the scenario where the output graph is hidden, by exploring the potential of learning from a random set of spanning trees.
In Section~\ref{sc_su11}, we start the discussion by presenting a majority voting ensemble (\mve) that combines a set of structured output classifiers.
...
In Section~\ref{sc_su14c}, we present and discuss the \rta\ method that improves \mam\ by performing joint learning and inference on a random set of spanning trees.
...
The model relies on the representation of the multiple output variables via a output graph which allows us to explore the label correlation.
To apply structured output learning, it is assumed the structure of the output graph is fully observed.
Alternatively in \citepub{su10}, we suppose there exists auxiliary data that encodes the label correlation which can be utilized in order to retrieve the output graph.
...
The Random Spanning Tree Approximation (\rta) model developed in \citepub{su14c} is a major step forward of \mam.
...
\citepub{su14c} is motivated by well-established Max-Margin Principle (Section~\ref{sc_svm}).
...
Each presented model has been implemented into a software package.
...
\begin{enumerate}
	...
	\end{enumerate}
	\item \mve\ developed in \citepub{su11} and \citepub{su14b} as well as \amm\ and \mam\ presented in \citepub{su14b} are ensemble models that combines a set of structured output classifiers built from random spanning trees.
	\begin{enumerate}
	...
	\end{enumerate}
	\item The structured output model used in \citepub{su10} for molecular activities classification is a instantiation of \mmcrf\ developed in \cite{Rousu07}.
	\begin{enumerate}
...
We focused on multilabel classification where the task is to predict multiple interdependent output variables given an input example.
...
The problem has been previously tackled by flat multilabel classification approach in which the multiple outputs are essentially as a ``flat'' vector.
The flat multilabel classification approach has difficulty of modeling the label correlation.
The structured output learning approach is a natural improvement to flat multilabel classification.
The first outcome of the thesis is to extend the applicability of the structured output learning methods.
We have applied the structured output learning in molecular activity classification problem.
We have also developed \spin\ that is able to predict a directed acyclic graph (\daggraph) that connects multiple output variables, and applied in network response prediction problem.
\spin\ can be also applied in other influence or spreading types problems.
We examine the performance of both methods.
The empirical results demonstrate that the structured output learning outperform the flat multilabel classification approach for predicting multiple interdependent output variables in structured output space.

The current structured output learning approach rely on representing the label correlation as an output graph known as \textit{a-priori}.
Limiting the applicability of the structured output prediction is the fact that the output graph is often unobserved.
The second outcome of the thesis is the new methods developed for structured output learning when the output dependency graph is hidden.
Analog but with significant difference to the ensemble method, the proposed methods combines a set of structured output classifiers.
In particular, \amm\ and \mam\ perform inference before or after combing the base learners that built on random trees, \rta\ performs joint learning and inference on a random set of spanning tree.
We have studied the behavior of the proposed methods in terms of reconstruction error of the compatibility score as well as the generalization error which guaranteed the performance of the proposed models.

In addition, the developed inference algorithms for the proposed structured output learning models can avoid the exponential parameter space and ensure the efficient learning.
...
First, the presented algorithm can be applied into other multilabel classification problems in structured output space.
