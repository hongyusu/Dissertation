...
The molecular activity classification problem has long been tackled under the single label classification setting.
...
Therefore, we investigated in \citepub{su10} the potential of using structured output learning framework to tackle the molecular activity classification problem.
...
To efficiently and accurately predict molecular activities in multiple cell line targets at the same time, we proposed a structured output learning algorithm in \citepub{su10}, which is to our knowledge the first multilabel learning approach for molecular classification problem.
...
In \citepub{su10}, We use graph kernel to measure the similarity of pair of molecular structures.
...
In \citepub{su10}, we also supposed the feasibility of extracting the structures although it is given implicitly in auxiliary datasets.
...
Therefore, we explored in \citepub{su11} the possibility of constructing an simple ensemble of multiple structured output learners built on random output graphs and applied the technique on molecular activity classification task.
...
We designed two approaches to generate random output graph.
...
Through extensive experiments in \citepub{su11}, we have validated the feasibility of constructing a simple majority voting ensemble (\mve) of structured output learner built on a set of random output graphs.
The proposed method also improved the performance on molecular activity classification problem.
In \citepub{su14b}, we aimed to develop elegant aggregation techniques (\amm\ and \mam\ respectively) that perform inference before or after forming ensemble.
...
\begin{definition}{Maximum of Average marginals Aggregation (\mam)}
	\begin{align*}
		...
		%\vy^* &= \underset{\vy}{\argmax}\, \frac{1}{T}\sum_{t=1}^{T}\sum_{e\in E_t}{w^t_e}^T\varphi_e(x,\vy)
		= \underset{\vy \in \Ycal}{\argmax}\, \frac{1}{T} \sum_{t=1}^T \sum_{e\in E} \ip{\vw_e^{(t)}}{\phi_e(x,\vy_e)}. %\label{ensemble3}
	\end{align*}
\end{definition}
With following lemma from \citepub{su14b}, we are able to simplify the computation of \mam\ in terms of dual variables and kernels.
\begin{lemma}
...
\end{lemma}
The proof can be found from \citepub{su14b}. 
...
In general, the theory described in \citepub{su14b} states that the reconstructive error from \mam\ ensemble is guaranteed to be less than or equal to the average reconstruction error from the base classifiers.
...
Thus, in addition to (Theory~\ref{}), we further provide theoretical justification of \mam\ that constructs a set of base learners on random spanning trees. 
Besides, \citepub[Theorem 5]{su14c} suggests we should optimize the joint margin from all spanning trees which leads to the \rta\ model described in the following section.
...
\begin{definition}{\bf Primal $L_2$-norm Random Tree Approximation (\rta).}\label{primalrta}
	\begin{align*}
		\underset{\vw_{T_t},\xi_k}{\minimize} & \quad \frac{1}{2}\sum_{t=1}^{n}\norm{\vw_{T_t}}_2^2 + C\sum_{i=1}^{m}\xi_i\\
		\st & \quad \sum_{t=1}^{n}{ \langle \vw_{T_t}, \phibh_{T_t}(x_i,\vy_i) \rangle} - \underset{\vy \neq \vy_i}{\maximize\ } \sum_{t=1}^{n}{\langle \vw_{T_t}, \phibh_{T_t}(x_i,\vy) \rangle } \geq 1 -  \xi_i, \\
		& \quad \xi_i\ge0\, , \forall\ i \in \set{1,\dots,m},
	\end{align*}
where $\phibh_{T_t}(\vx,\vy)$ is the feature map that is local on each tree $T_t$, $\xi_i$ is the margin slack allocated for each $x_i$, and $C$ is the slack parameter that controls the amount of regularization.
\end{definition}

The key for the optimization is to solve the '$\argmax$' problem efficiently.
This is a \nphard\ problem in practice, as the multilabel space is exponential in terms of the number of microlabels.
We proposed in \citepub{su14c} a $K$-best inference algorithm for the optimization problem working in $\Theta(Knk)$ time per data point, where $k$ is the number of microlabels in the multilabel vector and $K$ is the number of best multilabel we compute from each spanning tree.
...
\begin{align}\label{rta:maximizer_onetree}
	\hat{\vy}_{T_t}(x) = \underset{\vy\in\Ycal}{\argmax}\, F_{\vw_{T_t}}(x,\vy)\ \eqdef\ \underset{\vy\in\Ycal}{\argmax}\, \langle \vw_{T_t}, \phibh_{T_t}(x,\vy)\rangle,
\end{align}
can be solved in $\Theta(k)$ time by \textit{dynamic programming} also known as \textit{max-product}.
However, there is no guarantee the the maximizer of (\ref{rta:maximizer_onetree}) is also the global maximizer over all spanning trees.
Therefore, we compute for each random spanning tree $K$ best multilabel, which in total costs $\Theta(K\ell)$ time for all spanning trees.
Then we propose in \citepub{su14c} a lemma that is able to retrieve the best multilabel from $K$ best list in linear time.

Now the question is to make sure the global violation multilabel is within the list.
We provided a lemma in \citepub{su14c} to guarantee that with high probability the global violation multilabel is in the list when $K$ is not too large.

In addition, we derive the marginalized dual representation of definition~\ref{primalrta}
\begin{definition}{\bf $L_2$-norm RTA Marginalized Dual}\label{l2-marginaldual}\label{l2-md}
	\begin{align*}
...
The inference problem of \spin\ is \nphard\ in general which was solved by a semidefinite programming algorithm \sdp\ with approximation guarantee and a fast \greedy\ heuristics.
