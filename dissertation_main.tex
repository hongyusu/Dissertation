...
\begin{enumerate}
	...
	\item \mve\ presented in \citepub{su11} as well as \amm\ and \mam\ presented in \citepub{su14b} are the structured output learning algorithms that are not constrained to the availability of the output graph. The algorithms combine a set of structured output learners built on a set of random output graphs.
	\begin{enumerate}
...
In particular, the proposed algorithm \spin\ can predict a directed acyclic graph (\daggraph) from an observed underlying network that best ``responds'' to an input example.
The empirical evaluation on the network response prediction problem within the context of social network analysis demonstrates that the proposed model outperforms several flat multilabel classification approaches.
The study shows that accurate prediction can be achieved by structured output learning when the output dependency structure can be obtained and utilized during learning.
...
The applicability of the structured output learning is limited due to the fact that the output graph is assume to be known as \textit{a-priori}.
...
Analog but with significant differences to the previously established ensemble methods, the proposed algorithms aim to combines a set of structured output learners.
...
The model performs max-margin learning on a random sample of spanning trees.
The joint learning and inference in \rta\ ensures that the base learners built from a set of random spanning trees are optimized simultaneously towards the same global objective.
\rta\ also lays the foundation of performing inference on unknown graph structure to achieve fast optimization and accurate prediction with attainable computational efforts.

Besides the proposed structured output learning models, the thesis contains the theoretical studies which not only explain the improvement of the prediction performance obtained by the proposed algorithms but also provide the generalization error analysis.
...
First, the algorithms developed in this thesis can be applied to many other multilabel classification problems, with or without pre-established output graph.
Secondly, the algorithm development and theoretic analysis can be extended.
As the first direction of applying the proposed algorithms is clear and application oriented, we will concentrate on the algorithm development and theoretic analysis part.
...
Alternatively, the weight parameters can be put together by applying $L_1$-norm combination.
Learning with $L_1$-norm regularization of parameter combination has been previously studied under the multiple kernel learning setting \citep{Rakotomamonjy08simplemkl}, which can be formulated into an equivalent form of learning a weighted $L_2$-norm combination.
...
We also need to design new optimization techniques for $L_1$-norm regularization as the current alternative optimization strategy proposed by \citet{Rakotomamonjy08simplemkl} is not efficient.
Meanwhile, the theoretical properties should be extended from \rta\ to the new model. 
