...
The molecular classification problem has long been tackled by the single-label classification approaches \citep{Menchetti05weighted,Singh12qsar,Dutt12classification}.
On the other hand, multiple interdependent molecular activities are often screened simultaneously in drug research \citep{Shoemaker06the}, which presents two challenges for single-label classification.
...
To apply structured output learning, we extract output graphs from several auxiliary datasets in which the correlation between multiple activities is implicitly encoded.
