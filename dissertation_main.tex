...
\textit{Hinge Loss} which is a non-differentiable non-smooth function used in e.g., Support Vector Machines (\svm) \citep{Boser92,Cortes95support}
\begin{align*}
	\Lcal_{hinge}(\vy,f(\vx)) = \maximize (0,1-\vy f(\vx)),
\end{align*}
\textit{Hamming Loss} which is non-differentiable non-smooth function used in e.g. structured \svm\ \citep{THJA04,TJTA05}
\begin{align*}
	\Lcal_{0/1}(\vy,f(\vx)) = \ind{\vy=f(\vx)},
\end{align*}
\textit{Square Error Loss} which is a differentiable and smooth function applied in e.g., Ridge Regression by \citet{Hoerl00ridge}
\begin{align*}
	\Lcal_{square}(\vy,f(\vx)) = (\vy, f(\vx))^2,
\end{align*}
\textit{Exponential Loss} which is a differentiable and smooth function used in e.g., in \adaboost\ \citep{Schapire99improved}
\begin{align*}
	\Lcal_{exp}(\vy,f(\vx)) = \exp(\vy f(\vx)),
\end{align*}
\textit{Logistic Loss} which is a differentiable and smooth function used in e.g., in Logistic Regression \citep{Chen99}
\begin{align*}
