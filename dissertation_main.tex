...
As multiple output variables can be ``on'' and ``off'' simultaneously, various flat multilabel classification algorithms have been developed that aim to explore the correlation or the dependency between multiple output variables in order to make better prediction.
In particular, \citet{Tsoumakas07multi} summerized the established flat multilabel classification algorithms into two categories: problem transformation approach for example in \citep{Zhang05a,Read09classifier,Cheng09combining} and algorithm adaptation approach for example in \citep{Schapire99improved,Bian12corrlog}
...
For graph label or structured output learning in general, one inherited problem is the output dependency structure is assumed to be pre-established.
...
In particular, some hierarchical classification algorithms can also belong to graph labeling category, for example \citep{THJA04,Rousu06}.
...
In this thesis, we extend the applicability of structured output learning by developing new learning algorithms and applying them on real world multilabel classification problems.
...
The proposed method incorporate the output dependency structure into the learning process and is applied on the network response prediction problem.
...
In particular, Chapter~\ref{ch_sop} presents the structured output learning models developed for the multilabel classification problem, which extends the flat multilabel classification approach described in the previous chapter.
