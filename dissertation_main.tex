...
In this thesis, we study the \textit{supervised learning} in structure output space, also known as \textit{structured output prediction}.

\textit{Machine learning}, a active research field in information and computer science, has drawn enormous attentions during laster few decades, not only because it develops intelligent systems that generalize well from previous observed examples; but also because it is grounded from rich statistical learning theories that prove the feasibility and guarantee the performance.
...
\textit{Supervised learning}, one of the most important fields in \textit{machine learning}, is usually instantiated as learning a function that is capable of predicting the best value for the output variable $\vy$ given an observed input variable $\vx$.
...
Many excellent learning algorithms have been designed for the problems and achieved remarkable success in many applications.
...
Otherwise, the problem can be summarized as learning function for mapping from arbitrary input to arbitrary output.
...
On the other hand, methods that do not explicitly assume the output structure usually have less power of modeling the interdependency and often struggle in optimization.
...
In particular, the proposed model is capable of learning without predefined structure.
We focus on the efficiency and the scalability of the structured output prediction models, as the inference problem is ofter \nphard to solve.
Additionally, we provide the theory to explain the behaviour and to guarantee the performance of the models.
In particular, our research questions can be casted as followings:

\textbf{Question 1}: Is structured output learning models more suitable for predicting multiple interdependent variables in structured output space?

\textbf{Question 2}: Can we efficient compute the inference problem of the structured output learning model which is usually \nphard?

\textbf{Question 3}: How to tackle the problem in structured output learning when the dependency structure is unknown?

\textbf{Question 4}: Can we provide any theories to explain the improvement and to guarantee the empirical risk of the structured output models?
...
In addition to the main contribution, the thesis also proposes a new learning algorithm that is capable of predicting a directed acyclic graph and demonstrates the performance on influence network prediction problem.
...
Chapter~\ref{ch_models} introduces the novel statistical models developed by the authors.
In particular, Section~\ref{sc_su10} explains the structured output learning model for molecular activity prediction.
Section~\ref{sc_su11} and Section~\ref{sc_su14a} discuss three ensemble model (\mve, \amm, \mam) that tackle the problem in structured output learning when the output structure is unknown.
Section~\ref{sc_su14c} considers \rta\ model that is a major step forward of \mam.
Section~\ref{sc_su14b} introduces \spin\ for network influence prediction.
Finally, Chapter~\ref{ch_conclusion} concludes the thesis by answering the research questions and by point out possible future directions.

In this thesis, the ideas and the formalisms of proposed statistical models for structured output learning are explained.
...
The background information and the related work of some proposed models are sometimes elaborated in the thesis to help better understand its contribution to the community.
Empirical evaluations of the proposed models are in nowhere restated, rather, they can be found from original publications. 
...
Logistic regression is another important method in statistical machine learning and is closely related to the Perceptron (Section~\ref{sc_perceptron}) and the Support Vector Machine (Section~\ref{sc_svm}).
...
In fact, structured perceptron makes two strong assumptions that restricts its power and applicability.
...
%
% --------------------------
\chapter{Novel Statistical Methods}\label{ch_models}
...
Each section will mostly focus on one publication, and will start with introduction and background knowledge that is necessary to understand the content within the section.
An overview of the theoretical end technical novelties will also be briefly presented.

\section{Structured Output Learning for Molecular Activity Classification}\label{sc_su10}
...
To efficiently and accurately predict molecular activities in multiple cell line at the same time, we proposed a structured output learning algorithm in \citepub{su10}, which is to our knowledge the first multi-task learning approach for molecular classification problem.
...
\end{align*}
where $0<\lambda<1$Using exponential series or geometric series the walk kernel can be compute in cubic time \citep{Gartner03a}
\begin{align*}
...
In \citepub{su14a}, a structure output learning model for network response prediction problem is developed.
...
%
%
\chapter{Conclusions and Future Directions}\label{ch_conclusion}

In this thesis, we study supervised learning in structured output space of which the task is to predict multiple interdependent variables for an arbitrary input variable.
In particular, we focus on the problem of structured output learning when the output structure is not observed.
We propose a learning framework that is capable of optimizing on a set of random output spanning trees when the output graph structure is missing.
We provide rich theory to justify our learning framework as well as to guarantee the performance.
We also study the network influence prediction problem for which we proposed a new structure output prediction model that can predict a direct acyclic graph.
We focus on the optimization of the proposed models for which we usually avoid a exponential size parameter space and the \nphard inference problem.

We posed several research question at the beginning of the thesis which are served as the guide through the presentation of the work.
Section~\ref{sc_single} and Section~\ref{sc_multi} introduce the backgrounds for both single-task and multi-task classification settings as well as discuss the algorithms that are relevant to the thesis.
Section~\ref{sc_ensemble} presents a few ensemble approaches that are also close related to the algorithm framework developed in the thesis.
Chapter~\ref{ch_models} is devoted to bringing in novel statistical models for structured output prediction that are developed by the author.
We will revisit the research questions with the insights and the results we gain from the novel statistical models discussed in the thesis as well as detailed in the publications.
...
The structured output prediction models are more suitable for the problem as it can describe the interdependencies between the output variables and is more efficient compared to single-task models presented in Section~\ref{sc_single}.
Besides, we demonstrate the performance of the structured output prediction model in molecular activity prediction problem in \citepub{su10} and in network influence prediction problem in \citepub{su14a}.

\textbf{Question 3}: How to tackle the problem in structured output learning when the dependency structure is unknown?

As it is often difficult to get the output graph structure that describes the interdependencies of output variables, we have developed several learning algorithms which eventually lead to a general framework that enable guide us to learn from a set of random output spanning trees.
This is based on the observation that any graph structure can be expressed by a complete graph connecting output variables.
Thus, we transfer the learning with unknown structure into learning with a complete graph.
With extensive theories in \citepub{su14c}, we are able to show that with a set of random spanning trees we can approximate the margin achieved by the complete graph.
Therefore, we are able to develop efficient learning algorithm based on a collection of random spanning trees.

\textbf{Question 2}: Can we efficient compute the inference problem of the structured output learning model which is usually \nphard?

The difficulty of the proposed structured output prediction models lies in two aspect, exponential parameter space and \nphard\ inference.
To avoid the exponential parameter space, we use marginalize dual decomposition technique which reduces the parameter space to polynomial size in terms of the edges in the output graph.
To overcome the \nphard\ inference problem, we adopted different inference techinques.
We use loopy belief propagation for the inference problem defined on general graph with output a approximate solution.
We use dynamic programming for the inference problem defined on a tree where the exact solution is guaranteed.
We use $K$-best inference algorithm developed in \citepub{su14c} for the inference problem defined on a set of random spanning trees.
We have also developed \sdp\ inference algorithm for the inference problem defined on directed graph.


\textbf{Question 4}: It the proposed learning framework well motivated? Can we explain the improvement and guarantee the empirical risk of the proposed structured output prediction models?

The idea of learning from a random sample of spanning tree as output graph has been explained in \citepub{su14c} as an approximation to learning with a complete graph in terms of margin achieved on a set of training examples.
