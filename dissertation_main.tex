...
\section{Multilabel $K$-Nearest Neighbor (\mlknn)} \label{sc_mlknn}

\citet{Zhang05a,Zhang07mlknn} proposed Multilabel $K$-Nearest neightbor (\mlknn) for flat multilabel classification.
\mlknn\ is an instance based learning \citep{Aha91instance} that is derived from $K$-Nearest Neightbor (\knn) algorithm designed for single-label classification problem.
...
\citet{Tsoumakas07multi} proposed the concept of \textit{binary relevant classifier} that amounts to transform the multilabel classification problem into a set of single-label classification problems and to independently learn a single-label classifier for each microlabel.
...
%
%
\section{Instant Based Logistic Regression (\iblr)}
...
It extends \mlknn\ by exploring the label correlation.
For each unseen example $\vx$, \iblr\ first identifies the set of $K$-nearest neighbors $N_k(\vx)$ from the training sets.
Then the algorithm builds a logistic regression (\lr) model for each microlabel based on the label information collected from all training examples in $N_k(\vx)$.

Mathematically, given an unseen example $\vx$, \iblr\ defines a posterior probability for the $j$'th microlabel of $\vx$ being labeled as $u_j$ by
\begin{align*}
	\pi^{(j)} = P( \vy(\vx)[j]=u_j | N_k(\vx)), u_j\in\Ycal_j.
\end{align*}
Then \iblr\ constructs a logistic regression classifier for $\pi^{(j)}$ which can be derived from the following 
\begin{align*}
...
%
%
\section{Multitask Feature Learning (\mtl)}
