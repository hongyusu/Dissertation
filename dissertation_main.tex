...
Most importantly, there is no requirement for base learners to be strong as long as they perform better than random guessing.
...
There are several different type of ensemble algortihm, to name a few, bagging \citep{Breiman96bagging}, boosting \citep{Freund97a,Schapire99improved}, stacking \citep{Smyth99linearly}, Bayesian averaging \citep{Freund04generalization}.
...
A \textit{concept} is a boolean function over domain $\Xcal$, and a \textit{concept class} is a class of concept.
% TODO: an illustrative example
A concept class is \textit{strongly learnable} if there exists a polynomial learning algorithm that achieves high accuracy with high probability for all concept.
On the other hand, a concept class is \textit{weak learnable} if the learning algorithm achieves arbitrary high accuracy where the only requirement is that the learning algorithm find a function performs better than coin flip.
...
In particular, the algorithm computes in each iteration a weak learner $f^t(\vx)$ base on all training examples and the current distribution $D^{t}$ (line~\ref{compuet_f}), calculates the weighted training error $\epsilon^k$ for the current weak learner (line~\ref{compute_error}), and computes the adaptive parameter $\alpha^t$ for current weak learner (line~\ref{compute_alpha}).
...
To see this, we fist compute the exponential loss for $\alpha^tf^t(\vx)$ given current distribution $D^t$ over training example and adaptive parameter for $f^t(\vx)$
\begin{align*}
...
It is usually achieved by reweighing which initials a uniform distribution over training examples and updates the distribution after each iteration.
For learning algorithms that cannot handle training data with specific distribution, we can apply resampling which samples training examples in each iteration according to some desired distribution.

Most recently, a new boosting algorithm \deepboosting\ was developed in \citep{Cortes14deep}.
\deepboosting\ allows the learning algorithm to use complex hypothesis class contain very deep decision trees, by which it extends \adaboost\ that only uses simple hypothesis class of decision tree in depth one.
...
\adaboostmh\ is the multilabel variance of original \adaboost\ algorithm developed by \citet{Schapire99improved}, which is further developed by \citet{Esuli2008boosting}.
...
It maintains a distribution over all examples and labels.
...
\begin{algorithmic}[1]
	...
	\FOR{$t=1\cdots T$}
		...
		\STATE $D^t(i,l) = D^{t-1}(i,l)\exp(-\alpha^t\vy_i[l]\hat{\vy}_i[l]),\forall i\in\{1,\cdots,m\},\forall l\in\{1,\cdots,k\}$ \label{mh_update_d}
	\ENDFOR
	\RETURN $H(\vx) = \sign(\sum_{t=1}^T\alpha^tf^t(\vx))$
\end{algorithmic}
\end{algorithm}

Furthermore, many other ensemble approaches have been proposed and applied in specific areas, to name a few, in natural language processing \citep{Collins05distrimnative,Zeman05improving,Sagae06parsing,Zhang09kbest} and 
in speech recognition \citep{Fiscus97a,Benesty08speech,Petrov10products}.
Limiting the power of these approaches is the fact that they are mostly tailored for the application and it is not immediately clear how to apply them to other machine learning tasks.
In addition, \citet{Kocev13tree} have developed ensemble approaches based on bagging with the limitation that the methods can only use decision tree as base learner.
