...
First, the algorithms developed in this thesis can be applied to many other multilabel classification problems in which the output does not need to be observed but is believed to play an important role during learning.
Secondly, the development of the learning algorithms and the theoretical studies are readily to be continued.
As the first direction is straight-forward and application oriented, we will focus on the second one and explain it in detail.

To serve as a starting point, the inference algorithm for \spin\ can be further developed, which not only needs to be scaled to large social network datasets but also should have approximation guarantee.
The research on structured output learning with unknown output graph should also be continued.
In particular, we plan to investigate the possibility of learning the feature weight parameters defined on random spanning trees through $L_1$ combination.
