...
This chapter aims to explain the general scope the thesis, to propose several research questions, and to briefly review the contributions of the thesis.

%
%
\section{Scope of the Thesis}

Machine learning, as defined by Arthur Samuel ``a field of study that gives computers the ability to learn without being explicitly programmed'', has become an active research field in information and computer science during the past few decades.
...
In this thesis, we examine the potential of applying structured output learning to improve the prediction perforamnce in real world multilabel classification problems.
...
We investigate the efficiency and the scalability of the inference problems within the proposed learning models as the inference problem of the structured output learning is often difficult to solve.
Besides, we study the proposed model from the theoretic aspect where we wish to gain better understanding of the behaviour.
In particular, the research questions can be casted as following:
\begin{enumerate}[label=\textbf{Q \Roman*}:]
\item Is the structured output learning approach 

Is the structured output prediction model more suitable for predicting multiple interdependent variables in structured output space?
...
The contribution of the thesis includes but is not limited to developing structured output learning methods for multilabel classification problem.
In particular, we have designed new learning methods that incorporate the output dependency structure into the learning process.
The methods are applied on molecular activity prediction task and the network influence prediction task.
Besides, we have worked on the problem of the structured output learning where the output dependency structure is not observed.
We have designed the learning framework that does not required the observed output graph and can learn from a random sample of spanning trees.
...
Chapter~\ref{ch_rlc} give the background of learning for classification which is necessary to understand the later part of the thesis.
The chapter cover the basic concepts in classification learning include risk minimization and regularization in Section~\ref{sc_rrm}, single-label classification in Section~\ref{sc_slc}, ensemble methods in Section~\ref{sc_em}.
Chapter~\ref{ch_fmlc} introduces the multilabel classification problem which is the problem under study in this thesis.
One line of research for the problem known as flat multilabel classification is cover in this chapter.
Chapter~\ref{ch_sop} and Chapter~\ref{ch_sopw} contain the main contribution of the thesis.
In particular, Chapter~\ref{ch_sop} presents the structured output learning models developed for multilabel classification problem, and Chapter~\ref{ch_sopw} explains the several learning approaches that are developed for structured output learning when the output structured is not observed.
Chapter~\ref{ch_ecm} covers the evaluation metrics and statistic test methods that are used in the reserach.
Chapter~\ref{ch_implementation} cover the implementation details of the developed models.
...
The formalisms of the proposed methdos are also briefly explained.
...
Empirical evaluations of the proposed models are in nowhere repeated, rather, they can be found from the original research articles. 
