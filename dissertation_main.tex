...
For graph labeling or structured output learning in general, one inherited problem is the output graph is assumed to be observed as \textit{a-priori}.
...
For the general multilabel classification problem without pre-established output graph, we develop several new algorithms that combine a set of structured output learners built on a collection of random output graphs.
...
We assume that data are drawn from a domain $\vXcal\times\vYcal$, where $\vXcal$ is the set of inputs (instants) and $\vYcal$ is the set of outputs (multilabels).
...
\citepub{su14c} presents Random Spanning Tree Approximation (\rta) for structured output learning where the output graph is not observed but believed to play an important role.
...
\citepub{su14c} also presents the theoretical studies which not only explain the intuition behind the learning model but also provide the generalization error analysis.
...
In particular, we construct a structured output prediction model with a complete graph as the output graph and rely on the optimization algorithm to reveal the hidden ``parameters'' defined on the edges (e.g., edge potentials).
