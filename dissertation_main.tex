...
\begin{definition}{Positive Semi-definite Kernel.}\label{psdkernel}
	Given a non-empty set $\Scal=\{x_1,\cdots,x_m\}\subseteq\Xcal$ of $m$ objects, a function $k:\Scal\times\Scal\rightarrow\RR$ is called a positive semi-definite kernel if it generates a positive semi-definite Kernel matrix $K$ with respect to $\Scal$.
\end{definition}
\noindent
...
First, the property ensures kernel based methods will converge to a relevant solution in in convex optimization \citep{Boyd2004convex}.
Secondly, it is a key assumption in \textit{Reproducing Kernel Hilbert Space} (\rkhs) theory described as follows.
