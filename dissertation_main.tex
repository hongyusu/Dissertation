...
In this case, one can only hope to gain addition performance from the label features with high correlation to the output label.

Besides, the \cc\ algorithm has been explained in terms of condition probability theory by Probabilistic Classifier Chains (\pcc) presented in \citep{Read09classifier}.
Since there is no standard way for the classification order of microlabels in \cc, Ensemble Classifier Chains (\ecc) developed in \citep{Read11classifier} further improves \cc\ by generating and combining multiple chains of classifiers. 
