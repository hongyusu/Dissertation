...
\begin{align*}
	\sum_{i,j=1}^{m}c_ic_jk(\vx_i,\vx_j) = \sum_{i,j=1}^{m}c_ic_j\ip{\varphib(\vx_i)}{\varphib(\vx_j)} = ||\sum_{i=1}^{m}c_i\varphib(\vx_i)||^2\ge0.
\end{align*}
...
The simplest kernel is \textit{linear kernel}
\begin{align*}
	k_{\text{\sc linear}}(\vx_i,\vx_j) = \ip{\vx_i}{\vx_j},
\end{align*}
which assumes a linear feature map $\varphib_{\text{\sc linear}}:\Xcal\rightarrow\Xcal$.
...
\end{definition}
\noindent
It has been shown that Kernel matrices are positive semi-definite \citep{Taylor04kernel}.
