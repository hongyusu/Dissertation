...
%
%------------------------------------------------
\chapter{Multilabel Classification} \label{ch_mlc}

Multilabel classification is a nature extension to single-label classification presented in Chapter~\ref{sc_slc}.
In multilabel classification each input (instant) is simultaneously associated with multiple outputs (labels). 
The research of multi-label classification has progressed rapidly in the past two decades with many learning algorithms being developed and applied in practice.
In this chapter, the author wish to discuss the multilabel classification problem by introducing two families of algorithms.
The first family is known as flat multilabel classification by which the multilabel is treated as a ``flat'' vector.
The second family is known as structured output prediction by which the interdependency of multiple labels is models via an output graph that connecting multiple labels.
An improvement to structured output prediction is presented in the later part of this chapter which allow the structured output learning works with unknown output structure.
Thus, it broadens the applicability of structured output learning.
