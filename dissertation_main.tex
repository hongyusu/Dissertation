...
It is quite often difficult and expensive to get labeled data. 
In order to maximize the outcomes of the valuable experimental data, the common approach adopted in practice is known as \textit{K-fold cross-validation}.
...
The examples in the $k$'th subset is then taken as testing data of which the label is assumed to be unknown.
The resulting classifier is used to predict the examples in the $k$'th subset.
...
As most learning algorithm explicitly assume that the training data and the testing data are from the same distribution,
it is worth noticing that we should make sure that the property of training data is well preserved when partitioning the original training data into subsets.
...
To this end, we use \textit{stratification} or \textit{stratified sampling} where we first group training examples into equivalent classes based on the number of positive labels they have. 
Each class is then randomly split into $K$ local fold, after that local folds are merged to create $K$ global fold.
The stratification scheme ensure that also smaller classes have representations in all folds. 
...
\end{align*}
where by $\vy_i$ we denote the predicted label and by $\hat{\vy}_i$ we denote ground true label.
...
\end{align*}
where microlabel precision and recall are defined by
\begin{align*}
	Pre &= \frac{TP}{TP+FP}\\
	Rec &= \frac{TP}{TP+FN}.
\end{align*}
...
Once we compute the performance measures of multiple classifiers in terms of 0/1 loss, hamming loss and $F_1$ score, we want to compare the performance in order to decide the best model under consideration. 
To serve as a starting point, we assume a very basic setting where we already have the performance of multiple learning algorithms on one selected dataset.
It is then easy to compare the performance of the learning algorithms in terms of the performance measures without employing any statistical methods.
In practice, when a new learning algorithm is developed, it is usually evaluated over multiple dataset to rule out the probability that it performs well on one particular dataset by chance.
Statistical evaluation of experimental results have been considered as an essential part of comparison steps in such complicate scenario.
...
Another approach is to use paired t-test to check if the average difference of two learning algorithms is significantly different from zero.
Paired t-test is problematic due to commensurability and result will not be significant unless the sample size is larger enough (over 30 datasets).
...
They also suggest sign-test which details can be found from \citep{Demsar06statistical}.
...
This is equivalent to multiple hypothesis test.
One common way is use repeat-measures ANOVA which is base on two assumptions: first, the samples are drawn from normal distribution; second, the variants should be the same.
It is not difficult to see that the ANOVA is ill-posed as none the assumptions are met within the context of performance of learning algorithms over datasets.

% to test if pair of algorithms are significantly different from each other 
To overcome the problem, Friedman test \citep{Friedman37the,Friedman40Comparison} with post-hoc Nemenyi test \citep{Nemenyi63distribution} are proposed in \citep{Demsar06statistical}.
The goal of the combination test is to examine whether a pair of algorithms are significantly different from each other under multiple hypothesis test setup.
To perform Friedman test, we first compute the average rank $R_j$ of the $j$'th algorithms.
The Friedman test compares the average ranks and under null-hypothesis states that the average ranks are equal.
...
As the post-hoc Nemenyi test is usually conservative and has little statistical power, we sometimes use Bonferroni-Dunn test \citep{Dunn61mulitple} to compare the learning algorithms against the selected control algorithm.
The test results are usually visualized as \textit{$CD$ diagram}.
