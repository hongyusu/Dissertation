...
It extends \mmmn\ by defining the joint feature map as a tensor product between the input and the output feature map, and by developing an efficient optimization strategy. 
\mmcrf\ is applied in \citepub{su10} where the task is to reliably predict multiple interdependent molecular activities.

In particular, \mmcrf\ uses the exponential family to model the conditional probability of an multilabel $\vy$ given an input example $\vx$
\begin{align*}
	P(\vy|\vx) \propto \exp(\ip{\vw}{\phib(\vx,\vy)}) = \prod_{e\in E}\exp(\ip{\vw_e}{\phib_e(\vx,\vy_e)}),
\end{align*}
where $\phib_e(\vx,\vy_e)=\varphib(\vx)\otimes\Upsilonb_e(\vy_e)$ is the tensor product between an input feature map and an output feature of a multilabel $\vy$ on an edge $e\in E$ in an output Markov network $G$.
