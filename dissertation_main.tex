...
In this thesis, we study the supervised learning in structure output space, also known as structured output prediction.

\textit{Supervised learning}, one of the most important fields in \textit{machine learning}, is usually instantiated as learning a function that is capable of predicting the best value/values for the output variable $\vy$ given an observed input variable $\vx$.
...
We investigate the potentials of developing novel structured output learning models to improve the prediction performance in real world multi-task classification problems (e.g. molecular activity prediction, network influence prediction).
%We start with molecular activity prediction problem where the task is to predict multiple anti-cancer potentials given a drug-like molecules.
%We also study the network influence prediction problem where the goal is to predict the response subnetwork (e.g. a group of people in social network) from the underlying network with nodes performing the action and edges relying the action given an action (e.g. a posted message) performed on the network.
We focus on the efficiency and the scalability of the structured output learning models, as the inference problem is ofter \nphard to solve.
Additionally, we wish to provide the theory to explain the behaviour and to guarantee the performance of the models.
