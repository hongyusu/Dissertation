...
Additionally, we assume a feature map $\phib:\Xcal\rightarrow\Fcal$, which embeds the input in some high dimensional feature space $\Fcal=\RR^d$.
...
The model assumes a score function $\ip{\vw}{\phib(x,\vy)}$ as the inner product between a joint feature map $\phib(x,\vy)$ and some feature weight parameter $\vw$.
...
\begin{align}
	\hat{\vy} = \underset{\vy\in\Ycal}{\argmax} &\quad \ip{\vw}{\phib(x_i,\vy)}.  \label{perceptroninference}
\end{align}
...
\begin{align*}
	\vw \leftarrow \vw + \left(\phib(x_i,\vy_i) - \phib(x_i,\hat{\vy}_i)\right).
\end{align*}
...
\begin{algorithmic}[1]
	...
	\FOR{$t=1\cdots T$}
		\FOR{$i=1\cdots m$}
			\STATE $\hat{\vy} = \underset{\vy\in\Ycal}{\argmax} \quad \ip{\vw^{t-1,i-1}}{\phib(x_i,\vy)}$
			\IF{$\hat{\vy}\neq\vy$}
				\STATE $\vw^{t,i} = \vw^{t-1,i-1} + \phib(x_i,\vy_i) - \phib(x_i,\hat{\vy}_i)$
			\ENDIF
			\STATE $c=c+1$
		\ENDFOR
	\ENDFOR
...
\begin{align}
	L(\vw) = \sum_{i=1}^{m}\left[\ip{\vw}{\phib(x_i,\vy_i)}-\log{\sum_{\vy'\in\Ycal}{\exp{\ip{\vw}{\phib(x_i,\vy')}}}}\right] - \frac{1}{\sigma^2}\norm{\vw}^2 + C. \label{crf_inference}
\end{align}
...
\begin{definition}{Primal \mmmn\ optimization problem.}\label{def_mmmn}
	\begin{align*}
		\underset{\vw,\vxi}{\minimize} & \quad\frac{1}{2}\norm{\vw}^2 + C\sum_{i=1}^{m}\xi_i \\
		\st & \quad\ip{\vw}{\phib(x_i,\vy_i)} - \ip{\vw}{\phib(x_i,\vy)} \ge \vell(\vy_i,\vy)-\xi_i,\\
...
\begin{align}
	\hat{\vy} = \underset{\vy\in\Ycal/\vy_i}{\argmax} &\quad \ip{\vw}{\phib(x_i,\vy)} + \vell(\vy_i,\vy) \label{mmmninference}.
\end{align} 
...
\begin{definition}{Primal \svmstruct\ optimization problem.}\label{def_svmstruct}
	\begin{align*}
...
\begin{align}
	\hat{\vy} = \underset{\vy\in\Ycal/\vy_i}{\argmax} &\quad [1-\ip{\vw}{\phib(x_i,\vy)}]\,\vell(\vy_i,\vy) \label{svmstructinference},
\end{align}
...
That is the base classifier defines a compatibility score $\psi(\vx,\vy)$ for pairs $(\vx,\vy)\in\Xcal\times\Ycal$ according to the output graph $G$, indicating how well the input and the output goes together
\begin{align*}
