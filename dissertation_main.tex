...
\begin{definition}{Kernel Function.}
	A \textit{kernel} is a function $k$ that for all $x_i,x_j\in\Xcal$ satisfies
		$k(x_i,x_j) = \ip{\varphib(x_i)}{\varphib(x_j)},$
	where $\varphib:\Xcal\rightarrow\vFcal$ is a function that maps from an input space $\Xcal$ to a feature space $\vFcal$.
\end{definition}
\noindent
...
\begin{align*}
	{K}(\vx_i,\vx_j) = \frac{{K}(\vx_i,\vx_j)}{\sqrt{{K}(\vx_i,\vx_i){K}(\vx_j,\vx_j)}}
\end{align*}
to make sure that all elements in the kernel matrix lie on a unit hypersphere.

\begin{definition}{Positive Definite Matrix.}
	A $m\times m$ symmetric matrix $K$ is called positive definite if it satisfies $\sum_{i,j}c_ic_jK_{i,j}\ge0$ for all $c_i\in\RR$.
\end{definition}
