...
To serve as a starting point, the inference algorithm for \spin\ can be further developed, which not only needs to be scalable to large social network data but should also have an approximation guarantee.
Secondly, we plan to study \rta\ in multilabel classification problems with a general output graph.
The setting is interesting as output variables in many real world problems are usually organised by a graph structure which is more complex that a spanning tree or a chain but is less complex than a complete graph.
...
The additional weight can be seen as the fitness of the spanning tree to the current training data, which can be used to select relevant output structures.
