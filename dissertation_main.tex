...
The chapter starts by introducing in Section~\ref{sc_emp} two fundamental concepts, empirical risk minimization and regularization, in statistical machine learning.
...
\section{Regularized Risk Minimization}
...
Without loss of generality, we assume two random variable $\vx\in\vXcal$ and $\vy\in\vYcal$ are joint distributed according to some fixed but unknown probability distribution $P(\vx,\vy)$ over domain of $\vXcal\times\vYcal$.
...
A \textit{hypothesis class} $\Hcal$ is a set of functions learning algorithm is allowed to search against.
...
Thus, the estimator will generalize poor on unseen test examples.
...
The $L_2$ norm regularization defined by $\Omega(f) = ||\vw||^2_2$ will controls the complex of $f$ and provides a smooth solution, for example, ridge regression \citep{Hoerl00ridge}, logistic regression \citep{Chen00}, \svm \citep{Cortes95support}.
...
Additionally, we assume a feature map $\varphib:\vXcal\rightarrow\Fcal$, which embeds the input in some high dimensional feature space $\Fcal=\RR^d$.
...
\begin{theory}[Perceptron Convergence Theorem \citep{Block62the,Novikoff62}]\label{perceptron_theory}
	...
	Suppose for some $\gamma>0$ there exists a unit length vector $\norm{\hat{\vw}}=1$ such that $y_i(\ip{\hat{\vw}}{\varphib(\vx_i)})\ge\gamma$ for $i$ (training data $\Scal$ is linearly separable).
...
The prediction for a new test example $\vx_{ts}$ is computed by 
\begin{align*}
	y_{ts} = \sign\left(c^l({\sum_{l=1}^{k}{\ip{\vw^l}{\varphib(\vx_{ts})}+b^{l}}})\right) = \sign\left({{\ip{c^l\sum_{l=1}^{k}\vw^l}{\varphib(\vx_{ts})}+c^l\sum_{l=1}^{k}b^{l}}}\right),
\end{align*}
