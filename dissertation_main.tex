...
%
%------------------------------------------------
\chapter{Single-Label Classification}\label{ch_slc}
...
%
%
\subsection{Preliminary and Notations}\label{singlelabel_preliminary}
...
%
%------------------------------------------------
\chapter{Ensemble Methods}\label{ch_em}
...
%
%------------------------------------------------
\section{Preliminary and Notations}\label{sc_em_pn}
...
\subsection{Ensemble Methods for Multi-Label Classification}
...
However, it is not immediately apparent how to combine vector valued outputs in multi-label space.
...
%
%------------------------------------------------
\chapter{Multi-Label Classification} \label{ch_mlc}

Multi-label classification is a nature extension to the single-label classification presented in Section~\ref{sc_single}.
In multi-label classification each input (instant) is associated with multiple outputs (labels) simultaneously. 
The research in multi-label classification has progressed rapidly in the past few decades with many learning algorithms being developed and applied in practice.
In this section, the author wish to approach the multilabel classification problem by introducing two families of algorithms.
The first is known as flat multilabel classification by which the multilabel is treated as a ``flat'' vector.
The second family is known as structured output prediction by which the interdependency of multiple labels is models by an output graph.
...
%
%------------------------------------------------
\section{Preliminary and Notations}\label{multilabel_preliminary}

We borrow most notations from the single-label classification setting described in Section~\ref{singlelabel_preliminary}.
In particular, we examine the following \textit{multilabel classification} problem.
We assume data are drawn from a domain $\vXcal\times\vYcal$, where $\vXcal$ is a set and $\vYcal$ is the set of multilabels.
...
The goal is to learn a mapping function $f\in\Hcal:\vXcal\rightarrow\vYcal$ that can compute the output value of an arbitrary input such that the loss $\ell$ for the future unseen examples is minimized.
...
That is the interdependency is described by an \textit{output graph} that connects multiple labels.
In particular, we define the output graph $G=(E,V)$ via a node set $V=\{1,\cdots,k\}$ corresponding to the microlabels $\{y_1,\cdots,y_k\}$ and a edge set $E=V\times V$ representing the dependency between pair of microlabels.
...
%
%------------------------------------------------
\section{Flat Multi-label Classification}
...
%
% 
\subsubsection{Support Vector Machine for Interdependent and Structured Outputs (\svmstruct)}
...
%
%
\subsubsection{Multitask Feature Learning (\mtl)}
...
\subsection{Structured Output Prediction for Molecular Activity Classification}\label{sc_su10}
...
%
%
\subsection{\spin\ for Network Response Prediction}\label{sc_su14b}
...
%
%------------------------------------------------
\section{Structured Output Learning with Unknown Structure}

%
%
\subsection{Graph Labeling Ensemble (\mve)}\label{sc_su11}
...
\subsection{Random Graph Ensemble (\amm, \mam)}\label{sc_su14a}
...
% RTA
%
\subsection{Random Spanning Tree Approximation (\rta)}\label{sc_su14c}
...
%
%------------------------------------------------
\section{Evaluation and Comparison Metrics}\label{sc_statistics}
