...
The problem has been previously tackled by flat multilabel classification approach in which the multiple outputs are essentially treated as a ``flat'' vector.
The flat multilabel classification approach has difficulty of modeling the label correlation.
The structured output learning approach alleviates the problem via a output graph connecting multiple output labels.
The first outcome of the thesis is to extend the applicability of the structured output learning methods.
We have also developed \spin\ that is able to predict a directed acyclic graph (\daggraph) out from an observed network, and applied the model to network response prediction problem.
\spin\ can be also applied to other influence or spreading type problems.
The empirical results demonstrate that the structured output learning approach outperforms the previous flat multilabel classification approach for multilabel classification problem with observed output graph.
